...
def save_clustering_results(emb_name, algo, params,cluster_values,num=0,options=''):
    path = '/Users/mtileria/ApkProjects/general_framework/clustering-results/' + emb_name + '_' + num + '.json'
    if os.path.isfile(path):
        print('File exists, try different name')
        return
    output = {'algo':algo,'params':params, 'options':options, 'cluster': cluster_values}
...
def generate_docs_bert_C(df):
    ...
    for _,row in df.iterrows():
        if len(row['description']) > 0:
            ...
            description = lower_init(description)
            description = split_description(description)
            tmp = ' '.join([m_name_tokens,'from',class_tokens,description.lower()])
            docs.append(tmp)
            classification.append(row['classification'])
            keys.append('_'.join([row['class'],row['method_name']]))
    return keys,docs,classification
    ...
    for _,row in df.iterrows():
        ...
        if len(row['description']) > 0:
            ...
            if len(row['args_string']) > 0:
                arg_tokens = 'parameters are ' + ' '.join(get_params_token(row['args_string'],'string'))
            else:
...
def generate_docs_format_C(df):
    ...
    for _ ,row in df.iterrows():
        if len(row['description']) > 0:
            ...
            tmp = tmp + r + ' ' 
            if len(row['args_string']) > 0:
                arg_tokens = ' '.join(get_params_token(row['args_string'],'string'))
                tmp = tmp + arg_tokens
            docs.append(tmp.strip())
    return docs
...
def get_aosp_classes_methods(classes,methods,en_dict):
    ...
    methods_aosp = [method for method in methods if method not in en_dict]  #lower()
...
def get_qualify_name_gps(df_full,library,cl_name):
    query = df_full.loc[(df_full['lib'] == library) & (df_full['name'] == cl_name), 'fqname']
    if query.shape[0] == 0:
        return ''
    else:
...
def get_aosp_map():
    with open('/home/jovyan/work/inputs/classes_aosp_cleaned.txt') as f:
